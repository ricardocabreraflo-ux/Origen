...
exports.handler = async (event, context) => {
  ...
  if (!payload.id) {
    return { statusCode: 400, body: JSON.stringify({ error: "invalid_request", message: "Falta el id de la apertura." }) };
  }

  try {
    const supabase = getServiceClient();
    const { error } = await supabase.from("schedule_openings").delete().eq("id", payload.id);
